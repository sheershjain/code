...
@router.get("/update-an-flat/{id}")
def update_flat(id: int, request: Request, db: Session = Depends(get_db)):
    flat = db.query(Flats).filter(Flats.id == id).first()
    return templates.TemplateResponse(
        "update_flat.html", {"request": request, "flat": flat}
    )
...
def show_flats_to_delete(request: Request, db: Session = Depends(get_db)):
    ...
    if token is None:
        ...
        return templates.TemplateResponse(
            "show_flats_to_update_delete.html", {"request": request, "errors": errors}
        )
    else:
        try:
            scheme, _, param = token.partition(" ")
            ...
            if email=="sheersh@gkmit.co":
                flats = db.query(Flats).all()
                return templates.TemplateResponse(
                    "show_flats_to_update_delete.html", {"request": request, "flats": flats}
                )
            else:
            ...
            return templates.TemplateResponse(
                "show_flats_to_update_delete.html",
                {"request": request, "errors": errors},
            )

@router.get("/search")
def autocomplete(request:Request,query: Optional[str] = None, db: Session = Depends(get_db)):
    flats = db.query(Flats).filter(or_(Flats.description.contains(query), Flats.monthly_rent.contains(query))).all()
    
    #flat = db.query(Flats).filter(Flats.monthly_rent.contains(query)).all()
    # flats+=flat
    return templates.TemplateResponse(
                "flat_homepage.html",
                {"request": request, "flats": flats},
            )